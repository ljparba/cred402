    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <span className={cn("grid h-7 w-7 place-items-center rounded-full border", meta.ring)}>
            <Icon className={cn("h-4 w-4", meta.text)} />
          </span>
          <span className="text-sm font-semibold text-ink">{check.label}</span>
        </div>
        <span className={cn("text-xs font-bold", meta.text)}>{check.status}</span>
      </div>
      <p className="text-xs leading-relaxed text-ink-dim">{check.evidence}</p>
      {hashscanUrl && (
        <a
          href={hashscanUrl}
          target="_blank"
